...
5. `penctl` limitations in release v0.4.0:
...
6. Networking support is not provided for: DCQCN, PFC, QoS/Scheduling
...
4. penctl: FEC options do not work in FreeBSD. FEC options work with AN disabled. Certain vendors like Ixia and Few Cisco switches support FEC being turned off with AN enabled, in case of cisco 3232 we need to set speed (AN off ) and that is release noted. Pensando behavior is to disable AN to change FEC
